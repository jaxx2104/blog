...
purchase-explorer が自分を止める設計にしているのは、過去分の取り込みには終わりがあるからです。毎週 1 回、まだ試していない検索クエリを 2〜3 個だけ試して、結果を状態ファイルに書いて前進します。3 回連続で新規ゼロになったら「探索完了」と報告して`hermes cron pause`を自分に対して実行します。
...
2026-08-02 に、メルカリの取引完了メール 76 件を Gmail から取り込みました。そこに、wiki にも自分の記憶にも存在しないデバイスが 2 つ入っていました。
