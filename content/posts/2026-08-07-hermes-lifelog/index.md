...
[Cline](/cline-review)が出てきたとき、これを一番使いこなせるのは物書きの人だろうと思っていました。文章を書ける人ほど有利に見えたからです。自分は物書きではないので、単純に焦りました。
...
1 つ目は OAuth クライアントの公開ステータスです。Testing のままだと refresh token が 7 日で強制失効します。毎朝動いているジョブが`invalid_grant`で落ちるまで気づきませんでした。
...
もう 1 つは Takeout のサイズです。エクスポートは 24 個の zip で 30GB 近くありますが、実際に取り込むマップの保存場所と保存リストは、そのうち 2 個に入った合計 55KB でした。最初のプロンプトは未処理の zip を全部ダウンロードしてから中身を見る作りだったので、写真のアーカイブを落としては捨てていました。取り込む対象は最初から決まっているので、サイズを見てから落とす順にしておけば済んだ話です。

Takeout 自体は月次エクスポートを予約してあり、Drive へ落ちた数日後に cron が拾います。マップの保存場所は 213 件、自分が用途別に分類した保存リストが 15 本。スターを付けた最古の記録は 2011-05-17 でした。

Calendar は wiki には落とさず、毎朝 08:00 の morning-brief がその日の予定として読むだけの扱いです。予定がゼロの日はセクションごと消えるので、繋がっているかどうかは見た目では分かりません。疎通確認は`google_api.py calendar list`の終了コードでやっています。
...
ここまでは集計です。面白いのはこの先で、買い替えが集中する時期まで書いてありました。2016〜2017 年に Watch・iPhone・MacBook Pro を 10 か月で買い替えて、2020〜2021 年に iPad Pro・M1 Air・13 mini でもう一度、2023〜2024 年に 3 度目。主要デバイスの更新は 3〜4 年おきに固まっていて、次は 2027 年前後だろう、と結んでいます。

「次の買い替えタイミング」を聞いたときに返ってくるのは、この周期と四半期の棚卸しレポートを突き合わせた結果です。棚卸しの方は「購入から 1 年以上経過していて手元にあるもの」を 62 件挙げています。上位に並ぶのは 7〜8 年目の照明やスマートスピーカーで、買い替えを気にしていたガジェットより先に生活まわりが挙がります。ここまで、こちらは何も説明していません。
...
2026-08-02 に、メルカリの取引完了メール 76 件を Gmail から読みました。売却の記録なので、上の 604 件には入っていません。そこに、wiki にも自分の記憶にも存在しないデバイスが 2 つ入っていました。
...
LLM から性能を引き出すのは物書きの人だ、という見立ては、たぶん半分は当たっていました。ただ自分の場合、抜けていた前提は素材の方でした。書けないことより、書く材料を手元に持っていないことの方が効いていた気がします。

素材の方は勝手に溜まっていました。注文確認メールも、マップに付けたスターも、家の温度計の値も、意図して書いた文章ではありません。それを`raw/`に置いたら、要約も相互参照も索引の維持もエージェントがやりました。

Apple デバイスの遍歴ページも、買い替えが集中する時期の分析も、四半期の棚卸しレポートも、自分は 1 文字も書いていません。
...
予想は 2 つあって、どちらになるか分かりません。1 つは消耗品を淡々と補充する保守的な動き。棚卸しに 8 年目のシーリングライトが並んでいるので、そのあたりを替えにいくかもしれません。もう 1 つは運動器具です。ただ、これは無理筋だと今書きながら気づきました。運動のデータを渡していないからです。Apple Watch Series 7 は 2025-10-30 に売っていて、その事実だけが wiki に載っています。
